...
function searchLiburTenagaMedis() {
...
async function loadDataPaginate(page = 1, isActive = true) {
    ...
    try {
        ...
        renderLiburTenagaMedisTable(
            dataActive?.data?.allLiburTenagaMedisPaginate || {},
            "dataLiburTenagaMedisAktif",
            true
        );
        ...
        const queryArchive = `
            query($first: Int, $page: Int, $search: String) {
                allLiburTenagaMedisArchive(first: $first, page: $page, search: $search){
                    data { 
                            id
                            tenaga_medis_id
                            tanggal
        ...
        renderLiburTenagaMedisTable(
            dataArchive?.data?.allLiburTenagaMedisArchive || {},
            "dataLiburTenagaMedisArsip",
            false
        );
    } catch (error) {
...
async function createLiburTenagaMedis() {
    ...
    const mutationLiburTenagaMedis = `
        mutation($input: CreateLiburTenagaMedisInput!) {
            createLiburTenagaMedis(input: $input) {
                id
                tenaga_medis_id
                tanggal
                jenis
    ...
    `;
    const variablesLiburTenagaMedis = {
        input: { tenaga_medis_id, tanggal, jenis, keterangan },
    };

    try {
        const resLiburTenagaMedis = await fetch(API_URL, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                query: mutationLiburTenagaMedis,
                variables: variablesLiburTenagaMedis,
            }),
        });

        const resultLiburTenagaMedis = await resLiburTenagaMedis.json();
        const dataLiburTenagaMedis = resultLiburTenagaMedis?.data?.createLiburTenagaMedis;
...
function openEditModal(
    ...
    document.getElementById("edit-doktor").value = tenaga_medis_id;
...
// Update
async function updateLiburTenagaMedis() {
    const id = document.getElementById("edit-id").value;
    const tenaga_medis_id = document.getElementById("edit-doktor").value;
    const tanggal = document.getElementById("edit-tanggal").value;
    const jenis = document.getElementById("edit-jenis").value.trim();
    const keterangan = document.getElementById("edit-keterangan").value.trim();

    showLoading();

    const mutation = `
        mutation($id: ID!, $input: UpdateLiburTenagaMedisInput!) {
            updateLiburTenagaMedis(id: $id, input: $input) {
                id
                tenaga_medis_id
                tanggal
                jenis
...
function renderLiburTenagaMedisTable(result, tableId, isActive) {
    ...
    `;
        ...
        if (window.currentUserRole === "admin" || window.currentUserRole === "doctor") {
            if (isActive) {
                ...
                <button onclick="hapusLiburTenagaMedis(${item.id})"
                    class="${baseBtn} bg-rose-100 text-rose-700 hover:bg-rose-200 focus:ring-rose-300">
                    <i class='bx bx-archive'></i> Archive
                </button>`;
            } else {
                actions = `
                <button onclick="restoreLiburTenagaMedis(${item.id})"
                ...
                <button onclick="forceDeleteLiburTenagaMedis(${item.id})"
...
// Hapus
async function hapusLiburTenagaMedis(id) {
    if (!confirm("Are you sure you want to add to the archive??")) return;

    showLoading();
    const mutation = `mutation($id: ID!){ deleteLiburTenagaMedis(id: $id){ id } }`;
    try {
        await fetch(API_URL, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
// restore
async function restoreLiburTenagaMedis(id) {
    if (!confirm("Are you sure you want to restore this data?")) return;

    showLoading();
    const mutation = `mutation($id: ID!){ restoreLiburTenagaMedis(id: $id){ id } }`;
    try {
        await fetch(API_URL, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
// force delete
async function forceDeleteLiburTenagaMedis(id) {
    if (!confirm("Are you sure you want to delete this data??")) return;

    showLoading();
    const mutation = `mutation($id: ID!){ forceDeleteLiburTenagaMedis(id: $id){ id } }`;
    try {
        await fetch(API_URL, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
