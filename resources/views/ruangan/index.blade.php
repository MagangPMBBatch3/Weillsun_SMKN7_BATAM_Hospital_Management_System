<x-app-layout>
    <x-slot name="header">
        <div
            ...
            <h2 class="text-2xl font-extrabold tracking-tight text-gray-800 dark:text-gray-100 flex items-center gap-3">
                <i class='bx bx-door text-3xl text-blue-500'></i>
                <span class=" tracking-wider">Room</span>
            </h2>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-3 w-full lg:w-auto">
                ...
                @if (auth()->user()->role === 'admin')
                    <x-primary-button x-data=""
                        x-on:click.prevent="$dispatch('open-modal', 'create-ruangan')"
    ...
    <div class="px-4 sm:px-6 lg:px-8 mb-4">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
            ...
            {{-- Modal CREATE --}}
            <x-modal name="create-ruangan" focusable>
                <div class="p-6">
                    <form onsubmit="event.preventDefault(); createRuangan()">
                        <h2 class="text-xl shadow-md p-4 rounded-md font-bold mb-4">Add New Room</h2>

                        <div class="space-y-3">
                            <x-input-label>Room Name</x-input-label>
                            <x-text-input id="create-nama_ruangan" type="text" placeholder="Enter Room Name..."
                                class="border p-2 w-full rounded" required />
                            
                            <x-input-label>Capacity</x-input-label>
                            <x-text-input id="create-kapasitas" type="text" placeholder="Enter Capacity..."
                                class="border p-2 w-full rounded" required />

                            <x-input-label>Rate per Day</x-input-label>
                            <x-text-input id="create-tarif_per_hari" type="text" placeholder="Enter Rate per Day..."
                                class="border p-2 w-full rounded" required />
                        </div>
            ...
            {{-- Modal EDIT --}}
            <x-modal name="edit-ruangan">
                <div class="p-6">
                    <form onsubmit="event.preventDefault(); updateRuangan()">
                        <h2 class="text-xl shadow-md p-4 rounded-md font-bold mb-4">Edit Room</h2>

                        <x-text-input type="hidden" id="edit-id" />

                        <div class="space-y-3">
                            <x-input-label>New Room Name</x-input-label>
                            <x-text-input id="edit-nama_ruangan" type="text" placeholder="Name"
