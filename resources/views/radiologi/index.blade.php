<x-app-layout>
    <x-slot name="header">
        <div
            ...
            <h2 class="text-2xl font-extrabold tracking-tight text-gray-800 dark:text-gray-100 flex items-center gap-3">
                ...
                <span class=" tracking-wider">Radiology</span>
            </h2>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-3 w-full lg:w-auto">
                ...
                @if (auth()->user()->role === 'admin' || auth()->user()->role === 'doctor')
                    <x-primary-button x-data=""
                        x-on:click.prevent="$dispatch('open-modal', 'create-radiologi')"
    ...
    <div class="px-4 sm:px-6 lg:px-8 mb-4">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
            ...
            {{-- Tabel Data Aktif --}}
            <x-table id="tableActive" :headers="['ID', 'patient', 'personnel', 'type', 'result', 'date', 'fee']" :requireRole="['admin','doctor']">
                <tbody id="dataRadiologiAktif"></tbody>
            </x-table>

            {{-- Tabel Data Arsip --}}
            <x-table id="tableArchive" class="hidden" :headers="['ID', 'Patient', 'personnel', 'type', 'result', 'date', 'fee']" :requireRole="['admin','doctor']">
                <tbody id="dataRadiologiArsip"></tbody>
            </x-table>
            ...
            {{-- Modal CREATE --}}
            <x-modal name="create-radiologi" focusable>
                <div class="p-6">
                    <form onsubmit="event.preventDefault(); createRadiologi()">
                        ...
                        <div class="space-y-3">
                            ...
                            <select
                                ...
                                name="create-nickname" id="create-nickname">
                                <option value="" class="text-gray-500 italic">Select Personnel</option>
                                @foreach ($AlltenagaMedises as $tenagaMedis)
                                    <option value="{{ $tenagaMedis->id }}">
                                        {{ $tenagaMedis->profile->nickname }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <x-text-input id="create-jenis" type="text" placeholder="Enter Type..."
                                class="border p-2 w-full rounded" required />

                            <x-input-label>Result</x-input-label>
                            <textarea id="create-hasil" required placeholder="Enter Result..."
                            ...
                            <x-text-input id="create-biaya" type="text" placeholder="Enter Fee..."
                                class="border p-2 w-full rounded" required />
            ...
            {{-- Modal EDIT --}}
            <x-modal name="edit-radiologi">
                <div class="p-6">
                    <form onsubmit="event.preventDefault(); updateRadiologi()">
                        <h2 class="text-xl shadow-md p-4 rounded-md font-bold mb-4">Edit Data</h2>

                        <x-text-input type="hidden" id="edit-id" />

                        <div class="space-y-3">
                            <x-input-label>New Personnel</x-input-label>
                            <select
                                class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                            ...
                            <x-input-label>New Type</x-input-label>
                            <x-text-input id="edit-jenis" type="text" placeholder="Enter Type..."
                                class="border p-2 w-full rounded" required />

                            <x-input-label>New Result</x-input-label>
                            <textarea id="edit-hasil" required placeholder="Enter Result..."
                                class="border p-2 mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"></textarea>
                            ...
                            <x-text-input id="edit-biaya" type="text" placeholder="Enter Fee..."
                                class="border p-2 w-full rounded" required />
                        </div>
    ...
    {{-- JS --}}
    <script src="{{ asset('js/radiologi/radiologi.js') }}"></script>
    <script></script>
